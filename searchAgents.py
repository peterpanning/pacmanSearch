...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        # Always move one position at a time
        # You start where you start, with no corners visited
        self.startState = (self.startingPosition, {corner: False for corner in self.corners})
        # You want to end up having visited all the corners
        self.goal = {corner: True for corner in self.corners} # {(1, 1): True, }
    ...
    def isGoalState(self, state):
        ...
        """
        return state[1].items() == self.goal.items()
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if (nextPosition) in self.goal.keys() and visitedCorners[(nextPosition)] == False:
                    visitedCorners[(nextPosition)] = True
...
def closest_food(position, foodList):
    ...
    for food in foodList:
        branches[food] = manhattanDistance(position, foodList)
    return min(branches, key=branches.get)
